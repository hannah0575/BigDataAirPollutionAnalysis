...
#Combine pollution (NO2, PM10, Ozone - average)
## Only aiming to see which sites have the greatest average pollution change over time 


YearlySitePollDATA %>% bind_rows(YearlySitePollDATA(z[1:7,], mean))
...
NSWmap <- ggplot() + 
          ...
          coord_sf(xlim = c(145, 155), ylim=c(-31, -37)) 
...
map_with_data <- NSWmap +
  geom_point(data = MeanYrPolluteData, aes(x = Long, y = Lat, color=Region, size=Pollution, group=Sites, alpha = 0.3)) +
   scale_radius()
...
map_with_animation <- map_with_data +
  ...
  ggtitle('Year: {frame_time}',
          subtitle = 'Frame {frame} of {nframes}')
num_years <- max(MeanYrPolluteData$year) - min(MeanYrPolluteData$year) + 1
animate(map_with_animation, nframes = num_years)
...
p <- ggplot(
MeanYrPolluteData, 
  aes(x = Long, y=Lat, size = Pollution, colour = Region)
  ) +
...
# rename colnames for raster
colnames(df) <- c('x', 'y', 'vals')

RasterPM10 <- setnames(TARGETDATA$PM10, old = c('Long','Lat',"PM10"), new = c('x','y',"vals"))
RasterOZONE <- setnames(TARGETDATA$OZONE, old = c('Long','Lat',"Ozone"), new = c('x','y',"vals"))
RasterNO2 <- setnames(TARGETDATA$NO2, old = c('Long','Lat',"NO2"), new = c('x','y',"vals"))
