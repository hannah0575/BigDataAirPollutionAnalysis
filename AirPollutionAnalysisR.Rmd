...
# reassemble with Site (1) and Pollutants (2) 
  ...
  if (is.null(target_tsdata[[pollutant]])) { 

    dates = lubridate::dmy(PollSites[["Date"]]) #makes "Date" a date class 

    target_tsdata[[pollutant]] = data.frame (Date = dates) 

  } else { 
    target_tsdata[[pollutant]][site] = PollSites[[colname]] 
  } 
...
###3.2.1 Map the average concentration of air pollution over time at differeint target sites
```{r}
...
fviz_pca_var(Pm10PCA, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             main = "PM10 PCA Highest Contributing Sites"
             )
...
grp <- as.factor(tSiteList$PM10)

fviz_pca_var(Pm10PCA, col.var = grp, 
...
dfdf = time_by_poll$NO2
pca = princomp(dfdf[2:ncol(dfdf)])
plot(pca)
biplot(pca)
...
#5. Make Point file into an SF object to begin spatial plot 
